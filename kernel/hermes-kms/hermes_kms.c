				   ...
				   bool notify)
{
	...
	bool log_color_change = false;
	struct drm_hermes_kms_frame_color logged_color = { 0 };
	...
	if (!color || memcmp(&output->frame_color, color, sizeof(*color)))
		damage = NULL;
	if (fb && color) {
		if (memcmp(&output->frame_color, color, sizeof(*color))) {
			output->color_state_change_count++;
			logged_color = *color;
			log_color_change = true;
		}
	...
	if (log_color_change) {
		const struct hdr_metadata_infoframe *hdr =
			&logged_color.hdr.hdmi_metadata_type1;

		if (logged_color.flags & HERMES_KMS_COLOR_HDR_VALID)
			drm_info(drm,
				 "%s colour state: colorspace=%u eotf=%u max_cll=%u max_fall=%u mastering_luminance=%u..%u\n",
				 output->output_name, logged_color.colorspace,
				 hdr->eotf, hdr->max_cll, hdr->max_fall,
				 hdr->min_display_mastering_luminance,
				 hdr->max_display_mastering_luminance);
		else
			drm_info(drm, "%s colour state: colorspace=%u, SDR\n",
				 output->output_name, logged_color.colorspace);
	}
...
static int hermes_kms_stats_show(struct seq_file *m, void *data)
{
	...
	for (i = 0; i < hdev->output_count; i++) {
		...
		seq_printf(m, "frame_update_count:    %llu\n",
			   output->frame_update_count);
		seq_printf(m, "color_state_changes:   %llu\n",
			   output->color_state_change_count);
		seq_printf(m, "color_flags:           0x%x\n",
			   output->frame_color.flags);
		seq_printf(m, "colorspace:            %u\n",
			   output->frame_color.colorspace);
		if (output->frame_color.flags & HERMES_KMS_COLOR_HDR_VALID) {
			const struct hdr_metadata_infoframe *hdr =
				&output->frame_color.hdr.hdmi_metadata_type1;

			seq_printf(m,
				   "hdr_metadata:          eotf=%u max_cll=%u max_fall=%u mastering=%u..%u\n",
				   hdr->eotf, hdr->max_cll, hdr->max_fall,
				   hdr->min_display_mastering_luminance,
				   hdr->max_display_mastering_luminance);
		} else {
			seq_puts(m, "hdr_metadata:          none\n");
		}
