...
UAPI v14 closes the capture metadata gap with `ACQUIRE_FRAME2`. The response
contains the existing frame descriptor and a colour snapshot from the same
atomic commit: connector colorspace, full-range RGB semantics, and the complete
DRM type-1 HDR metadata (including EOTF and mastering/content luminance). Check
`CAP_FRAME_COLOR` before using it. The old ioctl is unchanged and still cannot
interpret HDR safely. See [HDR capture contract](hdr-capture.md).

Connector state is retained by each atomic transaction, never queried from a
later live state during flush. Metadata-only changes include the primary plane,
advance the primary sequence, wake waiters, and invalidate damage. Clearing the
HDR blob publishes SDR state with zero HDR fields. Unsupported blob sizes,
metadata types, EOTFs, colorspaces and PQ without BT.2020 fail atomic check,
including TEST_ONLY, before capture state changes.

`hdr_enable=1 color_depth=10` advertises HDR and ten-bit input; the compositor
still chooses whether to enable it. Ten-bit SDR remains SDR. The driver exports
RGB without conversion; P010 conversion, HDR codec signalling, tone mapping and
remote display negotiation belong to the consumer. Implementation of the kernel
contract does not establish end-to-end HDR readiness in Hermes or other clients.
...
runs both over the generated bytes, so the tables below stay a tracked artifact
rather than a snapshot that quietly goes stale.

Which conformity rules `edid-decode` implements varies by version, so the script
splits what it enforces. It always checks what the parser *decodes* — that both
data blocks are present with PQ and BT2020 RGB when `hdr_enable=1`, that neither
is when it is off, and that no block has a checksum error — because that
describes the bytes rather than a rule set, and it is the part worth guarding.
The exact `--check` finding set below is compared only when the running
`edid-decode` matches the version that recorded it, and reported as
informational otherwise; an older parser reporting fewer findings is a
difference in the parser, not in the EDID.
...
- `hdr_enable` (advertise HDR; gates a CTA-861 EDID extension carrying both an
  ...
  default off, load-time only; use `color_depth=10` for ten-bit HDR)
...
Implemented: HDR advertisement and frame-associated static HDR colour metadata
through UAPI v14. `tests/hdr-capture.c` exercises the atomic capture contract in a
disposable VM. End-to-end compositor, GPU conversion, encoder and client display
validation remains required; see [HDR capture contract](hdr-capture.md).
