...
# Run a real EDID parser over the same generated bytes and compare its findings
# against tests/edid-conformity.expected. check-edid proves the generator agrees
# with the test; this proves it still agrees with edid-decode, which is the part
# a shared author cannot vouch for. SKIPs when edid-decode is not installed.
